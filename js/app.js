...
async function handleStartFastAnalysis() {
  if (!decodedAudioBuffer) {
    logMessage('解析対象のオーディオデータが読み込まれていません。', 'error');
    return;
  }
  ...
  progressStatusText.textContent = '周波数を高速解析中...';
  showLoading('周波数を高速解析中...', '全フレームのスペクトラムを抽出しています');
  ...
  try {
    ...
    progressStatusText.textContent = `解析完了 (${elapsedSeconds}秒)`;
    logMessage(
      `高速解析が完了しました (所要時間: ${elapsedSeconds}秒, ${visualizer.getRecordedBarHeights().length} フレーム)。`
    );

    saveBarHeightsButton.disabled = false;
    recordStartButton.disabled = false;
    if (recordStartButtonText) {
      recordStartButtonText.textContent = '高速再解析';
    }
    ...
    logMessage(`高速解析に失敗しました: ${error.message}`, 'error');
    progressStatusText.textContent = '解析エラー';
    recordStartButton.disabled = false;
    hideLoading();
  }
}
...
async function handleSave() {
  ...
  if (visualizer.isRecomputing) {
    showLoading('波形データを生成中...', '最新のパラメータで全フレームを計算しています');
    try {
  ...
  if (recordedHeights.length === 0) {
    logMessage('保存する波形データがありません。先に「高速解析開始」を実行してください。', 'warning');
    return;
  }
  ...
  try {
    if (selectedFormat === 'waveform') {
      ...
      logMessage('波形データを spectrum_data.txt として保存しました。');
    } else if (selectedFormat === 'sb3') {
      if (!baseSb3Content) {
        logMessage('ベースとなる .sb3 ファイルがまだ読み込まれていません。', 'error');
        return;
      }
      if (!selectedAudioFile) {
        logMessage('音源ファイルが選択されていません。', 'error');
        return;
      }

      await exportAsScratchSb3(
        baseSb3Content,
        selectedAudioFile,
        processedDataArray,
        settingsHeader
      );
      logMessage('波形データを audio_spectrum.sb3 として保存しました。');
    }
  } catch (error) {
    console.error('保存処理中にエラーが発生しました:', error);
    logMessage(`保存に失敗しました: ${error.message}`, 'error');
  }
}

const INITIAL_FILE_DISPLAY_TEXT = 'オーディオファイルを選択 (.mp3, .wav, .aac, .flac ...)';
...
async function handleAudioFileChange(event) {
  ...
  if (file) {
    ...
    logMessage(`音源ファイル「${file.name}」をデコード中...`);
    showLoading('音源を読み込み中...', '音声データをデコードしています');
    ...
    try {
      ...
      logMessage(
        `デコード完了 (再生時間: ${Math.round(decodedAudioBuffer.duration)}秒)。自動で高速解析を開始します...`
      );
      ...
      logMessage(`音源のデコードに失敗しました: ${decodeError.message}`, 'error');
      hideLoading();
    }
  } else {
...
function setupBarEditor() {
  ...
  function createEffectCardElement(effect, index = 0, totalCount = 1) {
    ...
    dragHandle.title = 'ドラッグして適用順序を変更';
    ...
    switch (effect.type) {
      ...
      case 'gradient': {
        ...
        barHint.textContent = 'バーをクリックでピン追加、ピンをドラッグで位置調整';
        ...
        previewBar.title = 'クリックしてカラーストップを追加';
        ...
        function renderPins() {
          ...
          stops.forEach((stop, sIdx) => {
            ...
            pin.title = `ストップ #${sIdx + 1} (${Math.round(stop.offset * 100)}%): ドラッグして移動`;
        ...
        function renderStopsList() {
          ...
          stops.forEach((stop, sIdx) => {
            ...
            stopDragHandle.title = 'ドラッグして色の順序を変更';
            ...
            delBtn.type = 'button';
            delBtn.title = stops.length <= MIN_STOPS ? '最低2色のストップが必要です' : 'このストップを削除';
            delBtn.textContent = '✕';
        ...
        distributeBtn.title = '全カラーストップを0%〜100%の間で等間隔に配置します';
      ...
      case 'shapeArray': {
        ...
        const shapeOptions = [
          { val: 'rectangle', text: '四角形 (LED)' },
          { val: 'circle', text: '丸 (ドット)' },
          { val: 'diamond', text: 'ひし形' },
          { val: 'image', text: 'カスタム画像' }
        ];
      ...
      case 'texture': {
        ...
        const modes = [
          { val: '3-slice', text: '3スライス (角丸保持)' },
          { val: 'stretch', text: 'ストレッチ (全体伸縮)' },
          { val: 'tile', text: 'タイル (リピート)' }
        ];
        modes.forEach((m) => {
          const o = document.createElement('option');
          o.value = m.val;
  ...
  function renderEffectCards() {
    ...
    if (barEditor.effects.length === 0) {
      ...
      emptyBox.textContent = 'エフェクトはありません。右上の「＋ 追加」から自由にエフェクトを追加できます。';
  ...
  if (applyBarToVisualizerButton && barEditorModal) {
    applyBarToVisualizerButton.addEventListener('click', () => {
      showLoading('バー画像を事前ベイク中...', '各高さのスプライトキャッシュを生成しています');
      ...
      setTimeout(() => {
        try {
          ...
          logMessage(`1〜${maxHeight}px のバー画像 (太さ: ${chosenWidth}px) を事前ベイクして適用しました。`);
        } finally {
          hideLoading();
          closeModalSmooth(barEditorModal);
        }
  ...
  if (downloadBarPngButton) {
    downloadBarPngButton.addEventListener('click', () => {
      ...
      logMessage('バー画像を waveform_bar.png としてダウンロードしました。');
    });
  }
  ...
  if (resetBarEditorButton) {
    resetBarEditorButton.addEventListener('click', () => {
      ...
      renderPreview();
      updateSettingsFromUi();
      logMessage('バー描画をデフォルトの単色設定に戻しました。');
    });
  }
...
async function initializeApp() {
  initTheme();
  showLoading('システム初期化中...', 'ベースファイルを読み込んでいます');
  resizeCanvas();
  ...
  try {
    ...
    logMessage('.sb3 のロードが完了しました。');
  } catch (error) {
    console.error('抜け殻.sb3の読み込みエラー:', error);
    logMessage(
      'ベースとなる .sb3 ファイルの読み込みに失敗しました。sb3形式での保存は無効化されます。',
      'error'
    );
