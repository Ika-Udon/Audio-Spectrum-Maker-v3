...
export async function exportAsScratchSb3(
  ...
  if (!baseSb3Content) {
    throw new Error('ベースとなる .sb3 ファイルが読み込まれていません。');
  }
  if (!selectedAudioFile) {
    throw new Error('音源ファイルが選択されていません。');
  }
  ...
  if (!JSZip || typeof JSZip.loadAsync !== 'function') {
    throw new Error('JSZip ライブラリの読み込みに失敗しました。');
  }
  ...
  if (!stageTarget) {
    throw new Error('Scratch プロジェクトのステージターゲットが見つかりませんでした。');
  }
