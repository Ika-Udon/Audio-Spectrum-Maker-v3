...
export const EFFECT_DEFINITIONS = {
  ...
  glow: {
    type: 'glow',
    name: 'グロー / Bloom',
    description: '発光と光彩拡散（ブルーム）効果を与えます',
    defaultParams: {
      color: '#38bdf8',
      blur: 10,
      intensity: 2
  ...
  gradient: {
    type: 'gradient',
    name: 'グラデーション',
    description: '多段カラーストップや角度調整可能な自由度の高いグラデーションを適用します',
    defaultParams: {
      stops: [
  ...
  shapeArray: {
    type: 'shapeArray',
    name: '図形・画像集合 (配列)',
    description: '丸・四角・ひし形や小画像を連続して積み上げたアレイを描画します',
    defaultParams: {
  ...
  rgbShift: {
    type: 'rgbShift',
    name: 'RGB Shift (色ズレ)',
    description: '赤・緑・青のチャンネルをずらしてサイバーパンク・レトロ風の演出を作ります',
    defaultParams: {
      shift: 3,
      angle: 0 // 0: 水平, 90: 垂直
    }
  ...
  texture: {
    type: 'texture',
    name: 'テクスチャ画像',
    description: 'アップロード画像をバーの塗りやオーバーレイとして利用します',
    defaultParams: {
      image: null,
...
export class BarEditor {
  ...
  addEffect(type) {
    ...
    if (!def) {
      throw new Error(`未知のエフェクトタイプです: ${type}`);
    }
